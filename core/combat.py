...
def resolve_atk_def_term(
    attacker: Unit, target: Unit, damage_instance: DamageInstance
...
def resolve_increased_damage_taken(
    target: Unit, damage_instance: DamageInstance
) -> float:
    ...
    increased_damage_taken_mult: IncreasedDamageMultipliers = (
        target.initial_stats.special_attributes[SpecialAttribute.INCREASE_DAMAGE_TAKEN]
        + target.additive_modifiers.special_attributes[
...
def calculate_damage(
    ...
    effective_atk: float = 0
    effective_def: float = 0
    negative_def: float = 0
    term1: float = 0
    effective_atk, effective_def, negative_def, term1 = resolve_atk_def_term(
        attacker, target, damage_instance
    )

    if True:  # TODO: check if have reversed assault
        bonus_increased_damage = resolve_reversed_assault(damage_instance, negative_def)
    ...
    increased_damage_taken: float = resolve_increased_damage_taken(
    ...
    if not is_stability_broken:
        non_critical_damage = non_critical_damage * (
            1
            - target.initial_stats.basic_attributes[StatType.STABILITY_DAMAGE_REDUCTION]
            / 100
        )
    ...
    crit_dmg_multiplier: float = (
        attacker.get_effective_critical_damage_multiplier(damage_instance.tags) / 100
    )
